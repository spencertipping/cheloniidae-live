...
var cheloniidae = d.rebase (function () {
  var   qw = s >$> s.split(/\s+/), merging_constructor = attributes >$> (attributes * (a >$> '@#{a} = ($1 || {}).#{a} || ($0 || {}).#{a}')).join (',').fn(),
                                  patching_constructor =     (c, k) >$> '#{c} ($_, {#{k}: $0})'.fn(),
...
//   position, direction, etc. all at a moment; you get a new turtle after a line is drawn.
       ...
       rotational_turtle: merging_constructor(qw('position direction complement pen')).ctor ({
                             pen: patching_constructor ('cheloniidae.rotational_turtle', 'pen'),
                       draw_line: distance >$> (this.queue.push (new cheloniidae.line (this.position, this.position + (this.direction * distance), this.pen)), this),
                           ...
                           pitch:    angle >$> ((this.direction ^ this.complement) |$>
                                                (axis >$> new this.constructor (this, {complement: this.complement.about (axis, angle), direction: this.direction.about (axis, angle)})))}),

                     pen: qw('color size opacity') |$> (ps >$> merging_constructor(ps).ctor (ps * (p >$> p.maps_to (patching_constructor ('cheloniidae.pen', p))) / d.init, {
